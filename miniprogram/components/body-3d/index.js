/**
 * 卡通 3D 人体：WebGL 几何体组合（等价 Three.js SphereGeometry / CylinderGeometry）
 * 不依赖外部 three 包，适配微信小程序 canvas type="webgl"
 */
...
Component({
  ...
  methods: {
    ...
    _syncTargets() {
      ...
      const waist = Number(this.properties.waist) || 75;
      const hip = Number(this.properties.hip) || 95;
      const thigh = Number(this.properties.thigh) || 55;

      const fatFactor = this._clamp(bodyFat / 100, 0.08, 0.55);
      const waistScale = 1 + fatFactor * 0.8;
      const hipScale = 1 + hip / 100;
      const thighScale = 1 + thigh / 100;
      const scaleBase = this._clamp(weight / height, 0.42, 1.15);
      const stretchY = this._clamp(height / 170, 0.88, 1.18);

      this._targets = {
        waist: { x: waistScale, y: 1, z: waistScale },
        hip: { x: hipScale, y: hipScale * 0.92, z: hipScale * 0.88 },
        thigh: { x: thighScale, y: 1, z: thighScale * 0.95 },
        base: scaleBase,
        stretchY,
      };
    },
